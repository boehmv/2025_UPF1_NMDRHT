...
# Title: swish_DGE_gencode.v42.SIRVomeERCCome.R
# Objective: Main script for running Swish - DGE mode
...
dir.create(file.path(mydir, "Swish", "DGE"), showWarnings = FALSE, recursive = TRUE)
dir.create(file.path(mydir, "Plots", "Swish", "DGE"), showWarnings = FALSE, recursive = TRUE)
...
for (i in 2:length(condition)){
  ...
  gy <- summarizeToGene(y)
  
  # Scale inferential replicate counts
  gy <- scaleInfReps(gy) # scales counts
  
  # Apply filtering step (default parameters for one condition)
  gy <- labelKeep(gy, x = "condition") # labels features to keep
  
  # Label spike-ins (ERCC and SIRV) as FALSE in keep column -> do not analyze them!
  mcols(gy)$keep <- mcols(gy)$keep & !str_detect(rownames(assays(gy)[["counts"]]), 'ERCC') & !str_detect(rownames(assays(gy)[["counts"]]), 'SIRV')
  
  set.seed(1)
  
  # Run the "real" swish analysis
  gy <- swish(gy, x="condition") # simplest Swish case
  ...
  rep_count_names <- c("gene_id", rep_count_names)
  ...
  count_table <- as_tibble(infMed, rownames="gene_id") 
  colnames(count_table) <- rep_count_names
  
  # Generate annotated final table
  gy_df_gencode <- as_tibble(mcols(gy)) %>% 
    dplyr::select(-tx_ids) %>% 
    left_join(gtf_gencode_df_short %>% filter(type == "gene") %>% dplyr::select(gene_id, gene_name, gene_type),
              by=c("gene_id" = "gene_id")) %>% 
    mutate(condition_2 = cond) %>% 
    left_join(count_table)
  
  # Generate output csv file
  message("Generate Swish DGE csv output file for ", paste(cond))
  write.csv(gy_df_gencode, 
            file=(file.path(mydir,
                            "Swish",
                            "DGE",
                            cond, 
                            paste0(paste(cond),
                                   "_vs_control_Swish_DGE_results",
                                   ".csv"))))
  
  # Stats printing
  message("Swish DGE results significant qvalue < 0.0001 for  ", paste(cond))
  print(table(mcols(gy)$qvalue < 0.0001))
  
  if (mycount == 2) {
    DGE_cat <- gy_df_gencode
  } else {
...
write.csv(DGE_cat, file=file.path(mydir, "Swish", "DGE", paste0("Swish_DGE_cat",
                                                             ".csv")))
...
DGE_cat_counts_absolute <- DGE_cat %>% 
  ...
  labs(x = "Number of identified genes (n)",
       ...
       caption=paste0("\nSwish-DGE - Filter: |log2FoldChange| > 1 & qvalue < 0.0001"))


DGE_ridgeplot_counts <- plot_grid(DGE_ridgeplot + 
                                    labs(title="Swish DGE - gencode.v42.SIRVomeERCCome") +
...
ggsave(file.path(mydir, "Plots", "Swish", "DGE", "Swish_DGE_summary.pdf"),
       width = 12,
       height=12,
       units = "cm",
       device=cairo_pdf,
...
writeLines(capture.output(sessionInfo()), paste0(mydir, "/Swish/DGE/Swish_DGE_session_info.", format(Sys.time(), "%Y%m%d.%H%M"), ".txt"))
