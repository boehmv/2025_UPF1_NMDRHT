...
# Title: ISAR_gencode.NMDRegHumanTxome
...
accepted_biotypes <- NMDRegHumanTxome.v1.1_tbl %>% distinct(gene_biotype) %>% pull(gene_biotype)
...
myFiles <- file.path(mydir, "Salmon_NMD", samples$sample, "quant.sf")
...
SwitchList <- importRdata(
		...
		isoformExonAnnoation = file.path("/home/volker/2023_UPF1project/NMDRegHumanTxome/NMDRegHumanTxome.v1.1.SIRVomeERCCome.gtf.gz"),
...
# Set WD
setwd((file.path(mydir, "ISAR_NMD")))
...
writeLines(capture.output(sessionInfo()), paste0(mydir, "/ISAR_NMD/ISAR_session_info.", format(Sys.time(), "%Y%m%d.%H%M"), ".txt"))
