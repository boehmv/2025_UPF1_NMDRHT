...
# Title: edgeR_DTE_NMDRegHumanTxome.v1.1.SIRVomeERCCome
# Objective: Standard edgeR pipeline on the transcript level giving differentially expressed transcripts (DTE) with NMDRegHumanTxome annotations and first output analyses
...
# Load shortend NMDRegHumanTxome information
NMDRegHumanTxome.v1.1_tbl <- read_csv("/home/volker/2023_UPF1project/NMDRegHumanTxome/NMDRegHumanTxome.v1.1_short.csv")

# Create folders if not exists
dir.create(file.path(mydir, "edgeR", "DTE_NMDRegHumanTxome"), showWarnings = FALSE, recursive = TRUE)
dir.create(file.path(mydir, "Plots", "edgeR", "DTE_NMDRegHumanTxome"), showWarnings = FALSE, recursive = TRUE)
...
files <- file.path(mydir, "Salmon_NMD", samples$sample)
...
# Plot BCV
pdf(file = file.path(mydir, "Plots", "edgeR", "DTE_NMDRegHumanTxome", "NB_dispersion_BCV_plot.pdf"))
...
# Plot QLDisp
pdf(file = file.path(mydir, "Plots", "edgeR", "DTE_NMDRegHumanTxome", "QL_dispersions_plot.pdf"))
plotQLDisp(fit)
dev.off()
...
for (i in 2:length(cond)){
  ...
  qlf_df <- as.data.frame(topTags(qlf, n = Inf, sort.by = "none")) %>% 
    ...
    left_join(NMDRegHumanTxome.v1.1_tbl) %>% 
    relocate(gene_id, gene_name, transcript_id, transcript_name, transcript_biotype) %>% 
  ...
  anno <- data.frame(GeneID = qlf_df$gene_id,
                     ...
                     TranscriptType = qlf_df$transcript_biotype,
                     FDR = num(qlf_df$FDR,
                               digits = 2, 
                               notation = "sci"))
...
write.csv(DTE_cat, file=file.path(mydir, "edgeR", "DTE_NMDRegHumanTxome", paste0("edgeR_DTE_cat_NMDRegHumanTxome",
                                                                           ".csv")))
