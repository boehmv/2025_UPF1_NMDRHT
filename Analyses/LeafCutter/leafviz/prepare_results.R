...
threeprime_intersect = all.junctions %>%
  ...
  left_join(threeprime_db, by=c("chr","start"), relationship = "many-to-many")
...
fiveprime_intersect =  all.junctions %>%
  ...
  left_join(fiveprime_db, by=c("chr","start"), relationship = "many-to-many")
...
for( clu in clusters ){
  ...
  fprime=cluster %>% left_join(fprimeClu, by=c("chr","start"), relationship = "many-to-many")

  # three prime splice sites
  tprime=cluster %>% left_join(tprimeClu, by=c("chr"="chr","end"="start"), relationship = "many-to-many")

  # both splice sites
  bothSS=cluster %>% left_join(bothSSClu, by=c("chr","start","end"), relationship = "many-to-many")
...
introns %>% 
  mutate(condition_2 = str_split_i(code, "_vs_", 2)) %>% 
  write.csv(file=file.path(dirname(results_file),paste0(code,"_introns.csv")))

clusters %>% 
  mutate(condition_2 = str_split_i(code, "_vs_", 2)) %>% 
  write.csv(file=file.path(dirname(results_file),paste0(code,"_clusters.csv")))

intron_summary %>% 
  mutate(condition_2 = str_split_i(code, "_vs_", 2)) %>% 
  write.csv(file=file.path(dirname(results_file),paste0(code,"_intron_summary.csv")))

cluster_summary %>% 
  mutate(condition_2 = str_split_i(code, "_vs_", 2)) %>% 
  write.csv(file=file.path(dirname(results_file),paste0(code,"_cluster_summary.csv")))
