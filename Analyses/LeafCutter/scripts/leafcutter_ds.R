...
meta$sample <- as.character(meta$sample) # added to ensure purely numerical samples are processed
...
map_clusters_to_genes=function(intron_meta, exons_table) {
    gene_df=foreach (chr=sort(unique(intron_meta$chr)), .combine=rbind) %dopar% {
        ...
        all_matches=rbind(three_prime_matches, five_prime_matches)[ , c("clu", "gene_name", "gene_id")]
    ...
    clu_df=gene_df %>% group_by(clu) %>% summarize(genes=paste(gene_name, gene_id, collapse = ","))
...
home_dir <- getwd()
writeLines(capture.output(sessionInfo()), paste0(home_dir, "/leafcutter_run_session_info.", format(Sys.time(), "%Y%m%d.%H%M"), ".txt"))
